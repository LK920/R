...
haddop.lib.path = list.files(path = c("../lib"), pattern = "jar", full.names = T);
hadoop.class.path = list.files(path = c("../lib"), pattern = "jar", full.names = T);
class.path = c(hive.class.path, haddop.lib.path, hadoop.class.path);
...
#스타트카의 배기량에 따른 운전자의 연소득 예측 회귀모델 생성 및 요약
